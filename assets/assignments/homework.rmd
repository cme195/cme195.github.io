...
your homework as report including a writeup and all relevant code, and 
...
procuding output of your code. Your answers should be as concise as possible 
...
and prepare your own writeup separately. If you choose to collaborate,
...
betweein matrix $A$ and the 4th column of $B$. 


##c. Factors [5pt] 

In this part of the exercise we use a built-in data set, `sleep`, 
on student's sleep. This data stores infomation on the 
...
For each 'carrier' report the route ('origin'-'dest') wth
the highest mean ratio of depature delay over air time. Now, you know
which flights not to take with a given carrier.


**Note**: Since, the dataset contatins missiing values, when computing the 
...
* The datasets contains characters "." whcih needs to be replaced
...
Fit a multiple linerar regression model including all the variables 'TV',
