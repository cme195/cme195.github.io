...
between matrix $A$ and the 4th column of $B$. 
...
in respective columns. You can learn more about this data set from the page,
...
The sum of even numbers in the Fibonacci sequence not greater than 1M is
1,089,154.
...
We use "year", "month", "day", "origin", "hour", and "time_hour"
to join `flights2` and `airlines`.
...
emissions <- emissions %>%
    rename(Emission_CO2 = `Total CO2 emissions from fossil-fuels and cement production (thousand metric tons of C)`)
emissions
```
