...
- generates graphs by adding building blocks, instead calling different 
functions to draw new layers on top,
...
- text (`geom_text()`, `geom_lebel()`, used for text labels)
...
```{r}
diamond.counts <- diamonds %>% group_by(color) %>% summarise(count = n())
diamond.counts
```
...
dsmall <- dsmall %>% 
  mutate(pred.price = predict(lm(price ~ carat, data = dsmall)))
