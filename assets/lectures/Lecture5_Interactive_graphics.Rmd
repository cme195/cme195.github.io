...
df$cols <- rep_len(c("orange", "blue", "green"), length.out = length(z))
...
```{r plotly-scatter3D-col}
dbl.helix <- data.frame(t = rep(seq(0, 2*pi, length.out = 100), 2)) %>%
...
plot_ly(dbl.helix, x = ~x1, y = ~y1, z = ~z, type = "scatter3d", mode = "lines", 
  ...
  add_trace(x = ~x2, y = ~y2, z = ~z+10, color = "purple") 
```
