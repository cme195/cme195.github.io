...
* Unsupervised learning deals with a task of **inferring latent (hidden) 
patterns and structures unlabeled data**.
* The goal is to understand the **relationships between
features or among observations**.
* There are **no special variables such as response or output variables**,
...
* Most of **real life datasets are now high-dimensional** e.g. genetic 
sequencing data, medical records data, user internet activity data etc.
* D.R. or feature extraction methods **can reduce the number of 
variables.**
...
![](./Lecture8_Unsupervised_Learning_files/halfSphere.png)

</div>

<div style="float: left; width: 51%; height: 70%">
![](./Lecture8_Unsupervised_Learning_files/pca.png)
</div>
...
* The principal axes matrix, $V$, contatins the eigenvectors of the covariance
...
* The standard devations of the features, used for scaling:
...
ggplot(data.frame(pve = pve, pc = 1:length(pve)), aes(x = pc, y = pve)) + 
  geom_bar(stat = "identity") + ylim(0, 100) + theme_classic() +
  xlab("Principal Component") + ylab("Percent of Variance Explained") 
```
...
df <- data.frame(state = rownames(USArrests), pca.res$x[, 1:2])
ggplot(df, aes(x = PC1, y = PC2)) + geom_text(aes(label = state)) +
  xlab(paste0("PC1, ", round(pve[1], digits = 2), "%")) +
  ylab(paste0("PC2, ", round(pve[2], digits = 2), "%")) +
  theme_classic() + coord_fixed(pve[2]/pve[1])
```

## Features Plot

```{r, fig.height=6, fig.width=12}
L <- sweep(pca.res$rotation, 2, pca.res$sdev, "*")
df <- data.frame(features = colnames(USArrests), L[, 1:2])
ggplot(df, aes(x = PC1, y = PC2)) + geom_text(aes(label = features)) +
  xlab(paste0("PC1, ", round(pve[1], digits = 2), "%")) +
  ylab(paste0("PC2, ", round(pve[2], digits = 2), "%")) +
  theme_classic() + coord_fixed(pve[2]/pve[1])
```
...
So another software could return this plot instead:
...
![](./Lecture8_Unsupervised_Learning_files/clusters.png)
</div>
...
![](./Lecture8_Unsupervised_Learning_files/birdsOfFeather.png)
</div>
...
![](./Lecture8_Unsupervised_Learning_files/right.gif)
...
<div style="text-align: center; height: 75%">
![](./Lecture8_Unsupervised_Learning_files/Image.jpg)
</div>

## Importing image to R

* Firs, we download the image:
```{r}
library(jpeg)
url <- "http://www.infohostels.com/immagini/news/2179.jpg"
dFile <- download.file(url, "./Lecture8_Unsupervised_Learning_files/Image.jpg")
img <- readJPEG("./Lecture8_Unsupervised_Learning_files/Image.jpg") 
(imgDm <- dim(img))
```
...
![](./Lecture8_Unsupervised_Learning_files/calderMobile.jpg)
</div>
...
![](./Lecture8_Unsupervised_Learning_files/dogDendro.png)

</div>

<div style="float: left; width: 50%; height: 100%">
![](./Lecture8_Unsupervised_Learning_files/hierarchicalCutoff.png)
</div>


## Hierarchical clustering algorithm

![](./Lecture8_Unsupervised_Learning_files/hierarchicalClusteringAlgorithm.png)
...
<div style="text-align: center; height: 75%">
![](./Lecture8_Unsupervised_Learning_files/clustering_distances.png)
</div>

##
Different ways to compute dissimilarity between 2 clusters:

![](./Lecture8_Unsupervised_Learning_files/linkages.png)


## Iris dataset

* We will use the Fisher's Iris dataset contatining measurements on 150 irises.
...
We see that this time the results are better in terms of the cluster assignement
agreement with the flower species classfication.
...
of the disimilarity between each pair of 14 colors on a 5-point scale. 
...
<div style="text-align: center">
![](./Lecture8_Unsupervised_Learning_files/colors.jpeg)
</div>
...
ekmanSim <- readRDS("./Lecture8_Unsupervised_Learning_files/ekman.rds")
print(ekmanSim)
...
* It has beeen widely applied in many fields including genomics, where
the method is commonly used in single-cell literature for viusalizing 
cell subpopulations.
...
from the sudy conducted by [Amir et al. (2013)]( http://www.ncbi.nlm.nih.gov/pubmed/23685480). 
...
path <- "./Lecture8_Unsupervised_Learning_files/healthyHumanBoneMarrow_small.csv"
dat <- read.csv(path)
```
...
- Many online tutorials and forums
